...
const DialogInfoMember = () => {
  ...
  const handleImageClick = (url: string) => {
    setZoomedImage(url); // Set zoomed image when clicked
    dispatch(actionClickDialogInfoMember(true));
  };

  const handleCloseZoom = () => {
    setZoomedImage(null); // Close zoom when clicked outside the zoomed image
    dispatch(actionClickDialogInfoMember(true));
  };
        ...
        <div className="flex min-h-full items-center justify-center text-center sm:items-center sm:p-0">
          ...
          >
            <div>
              ...
              <DialogTitle as="h3" className="text-xl font-semibold bg-green-600 text-white pl-6 py-4">
                ...
                <div className="flex gap-4 w-10/12 mx-auto mt-4">
                  <div className="flex flex-1 gap-2">
                    <div className="flex flex-col flex-1 gap-4">
                      <button
                        type="button"
                        aria-label="Zoom Permis"
                        className="h-60 border-2 border-green-600 rounded-2xl flex items-center justify-center"
                        onClick={() => handleImageClick(permisURL)}
                      >
                        <img src={permisURL} alt="Logo" className="h-full w-full object-contain" />
                      </button>
                    ...
                    <div className="flex flex-col flex-1 gap-4">
                      <button
                        type="button"
                        aria-label="Zoom Validation"
                        className="h-60 border-2 border-green-600 rounded-2xl flex items-center justify-center"
                        onClick={() => handleImageClick(validationURL)}
                      >
                        <img src={validationURL} alt="Logo" className="h-full w-full object-contain" />
                      </button>
                    ...
                    <div className="flex flex-col flex-1 gap-4">
                      <button
                        type="button"
                        aria-label="Zoom Assurance"
                        className="h-60 border-2 border-green-600 rounded-2xl flex items-center justify-center"
                        onClick={() => handleImageClick(assurranceURL)}
                      >
                        <img src={assurranceURL} alt="Logo" className="h-full w-full object-contain" />
                      </button>
