...
import { actionClickDialogDeleteEvent } from '../../../store/actionCreator';
import { AppDispatch, RootState } from '../../../store/store';
import deleteEvent from '../../../../api/directus/event/deleteEvent';
import readAllEventsByUser from '../../../../api/directus/event/readAllEventsByUser';

const DialogDeleteEvent = () => {
  const dispatch: AppDispatch = useDispatch();
  const open = useSelector((state: RootState) => state.dialogReducer.dialogDeleteEvent);
  const event = useSelector((state: RootState) => state.eventReducer);
  const eventTitle = useSelector((state: RootState) => state.prestationReducer.title);
            ...
            <div>
              ...
              <div className="my-12">
                <div className="flex flex-col gap-2 justify-center items-center">
                  <p className="text-2xl font-bold">{eventTitle}</p>
                  <p className="text-base italic font-medium">
                    Toutes les inscriptions liées à cet événéments seront supprimés, un mail d&apos;annulation sera envoyé aux participants
                  </p>
                </div>
              </div>
            </div>
