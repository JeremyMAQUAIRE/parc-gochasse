...
import createcategory from '../../../../api/directus/category/createCategory';
import {
...
const DialogCreateCategory = () => {
  const dispatch: AppDispatch = useDispatch();
  const open = useSelector((state: RootState) => state.dialogReducer.dialogCreateCategory);
  ...
  const handleClickCreateCategory = async () => {
    ...
    dispatch(actionClickDialogCreateCategory(false));
          ...
          >
            <div>
              ...
              <DialogTitle as="h3" className="text-xl font-semibold text-gray-900 bg-zinc-200 pl-6 py-4">
                Ajouter une catégorie de prestation
                {/* Crois de fermeture */}
                <button
                  type="button"
                  onClick={() => {
