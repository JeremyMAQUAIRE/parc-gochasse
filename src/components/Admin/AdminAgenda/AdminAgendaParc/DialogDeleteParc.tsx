...
import deleteGestionParc from '../../../../api/directus/parc/deleteGestionParc';
import readAllGestionParcByUser from '../../../../api/directus/parc/readAllGestionParcByUser';

const DialogDeleteParc = () => {
  const dispatch: AppDispatch = useDispatch();
  const open = useSelector((state: RootState) => state.dialogReducer.dialogDeleteParc);
  ...
  return (
    <Dialog
      open={open}
      onClose={() => {
        dispatch(actionClickDialogDeleteParc(false));
      }}
          ...
          >
            <div>
              ...
              <DialogTitle as="h3" className="text-xl font-semibold text-gray-900 bg-zinc-200 pl-6 py-4">
                Etes-vous sûr de vouloir supprimer ce parc?
                {/* Crois de fermeture */}
                <button
                  type="button"
                  onClick={() => {
            ...
            <div className="mt-5 mr-16 flex gap-4 justify-end">
              ...
              <button
                type="button"
                onClick={async () => {
                  await dispatch(deleteGestionParc(parc.id));
...
export default DialogDeleteParc;
