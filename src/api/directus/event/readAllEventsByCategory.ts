...
export default createAsyncThunk('FETCH_READ_ALL_EVENT_BY_CATEGORY', async (id: number, thunkAPI) => {
  const response = await axios.get(
    `https://api.gochasse.com/items/events?filter[id_category][_eq]=${id}&fields=*,id_prestation.description,id_prestation.title,id_prestation.aplicable_on,id_category.color,id_parcs.name&sort=start_time_event&sort=date_event`,
    {
      headers: {
